        ...
        <el-col :span="24" :xs="24">
            ...
            >
                <el-form-item label="仓库">
                    <el-select v-model="queryParams.storeId" placeholder="仓库"
                               style="width: 210px">
...
<!--                <el-form-item>
            ...
            <el-table
                ...
                <el-table-column align="center" class-name="small-padding fixed-width" label="库存总览">
                    <el-table-column label="物料号" prop="productNo">
                    </el-table-column>
                    <el-table-column label="货品名称" prop="productName">
                    </el-table-column>
                    <el-table-column label="货品数量" prop="productNum">
                    </el-table-column>
                    <el-table-column label="查看批次">
                        <template slot-scope="scope">
                            <el-button size="small" type="primary" @click="getBatchList(scope.row)">查看批次</el-button>
                        </template>
                    </el-table-column>
                </el-table-column>
            </el-table>
            <pagination v-show="total>0" :limit.sync="queryParams.pageSize" :page-sizes="[10,15,20]"
                        ...
                        :total="total" @pagination="getData"/>
        </el-col>
        ...
        <el-dialog :close-on-click-modal="false" :visible.sync="slmBatchVisible" title="批次详情"
                   width="80%">
            <el-table
                :data="productBatchList">
                <el-table-column align="center" class-name="small-padding fixed-width" label="批次总览">
                    <el-table-column label="仓库编号" prop="storeNo">
                    </el-table-column>
                    <el-table-column label="物料号" prop="productNo">
                    </el-table-column>
                    ...
                    <el-table-column label="入库数量" prop="batchNum">
                    </el-table-column>
                    <el-table-column label="已使用数量" prop="usedNum">
                    </el-table-column>
                    <el-table-column label="剩余数量">
                        <template slot-scope="scope">
                            {{ scope.row.batchNum - scope.row.usedNum }}
                        </template>
                    </el-table-column>
                    <el-table-column label="含税单价" prop="priceInTax">
                    </el-table-column>
                    <el-table-column label="不含税单价" prop="priceEnTax">
                    </el-table-column>
                </el-table-column>
            </el-table>
        </el-dialog>
...
<script>
import {getBatchByProductId, getBatchDetail, getExchangeRate, list, stockControl} from "@/api/admin/slm/slm"
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            list(_this.queryParams).then(resp => {
        ...
        getBatchList(product) {
            let _this = this;
            _this.slmBatchVisible = true;
            let param = {
                storeId: product.storeNo,
                productNo: product.productNo
            }
            getBatchByProductId(param).then(resp => {
                _this.productBatchList = resp.data;
            })
        },
        ...
        showSummary(){
            this.$router.push('/admin/slm-summary/index/' + this.queryParams.storeId)
        },
        showPrint(){
            window.print();
        }
